...
public :
    ...
    /**
     * @brief Realiza la transformación lineal para ajustar el contraste de una imagen
     * @param z
     * @param a
     * @param b
     * @param min
     * @param max
     * @pre 0 <= (z,a, b, min, max) >= 255 && a < b && min < max
     * @return El valor de la transformación lineal
     * @post Se redondea el valor obtenido
     */
    double TransfLineal (byte z, byte a,byte b, byte min,byte max);
     ...
     * @param height
     * @param width
     * @pre 0 <= (i, j, height, width) && (i,height) <= filas && (j,width) <= columnas && i < height && j < columnas
     * @return La media de la submatriz con esquina superior izquierda (i,j) y height filas y height columnas
     * @post El valor de la media obtenido se redondea
     */
     ...
     * @post La imagen no se modifica.La imagen resultante tendrá tamaño int(filas/factor) x int(columnas/factor).
     ...
     * @post El objeto que llama al método contiene ahora una nueva imagen igual que la anterior pero
        con las filas ordenadas según el siguiente algoritmo: newr = (r*p) % rows
     */
    void ShuffleRows();
} ;
